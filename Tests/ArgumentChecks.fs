...
module ArgumentTests =
    let [<Fact>] ``verifying ignored wrong arguments should be accepted``() =
    ...
    let [<Fact>] ``verifying wrong arguments should throw an ExpectationViolationException``() =
        let mock = FsMockRepository()
        let list:int IList = mock.strict []
        mock.define Unordered {
            list.Add(1)
    ...
    let [<Fact>] ``verifying correct arguments``() =
        let mock = FsMockRepository()
        let list:int IList = mock.strict []
        mock.define Unordered {
            list.Add(3)
    ...
    let [<Fact>] ``verifying argument constraints``() =
        let mock = FsMockRepository()
        let list:int IList = mock.strict []
        mock.define Unordered {
